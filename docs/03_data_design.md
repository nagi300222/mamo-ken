...
> **v2.1: ギュイーン（競り合い）のミニゲーム化（v0.7実装のターゲット仕様）**。
> 三択(上/右下/左下)をジャンケン(✊グー＞✌️チョキ＞✋パー＞✊)に置き換え、ギュイーン発生ごとに
> **ジャンケン50% / レンダ25% / ハヤウチ25%** の3種からランダム選択する。突入演出26F中は
> ルーレットテロップでミニゲーム名を表示。勝敗リワードは3種共通（詳細は §6.5）。
>
...
| CLASH | 発生条件: 同段攻撃の振り合い±8F or 見切り成功 or 同一F同段 or つかみ同士。突入26F(ルーレットテロップ)→ミニゲーム(§6.5)→reveal60F。リワード共通: dmg120+連勝ごと+30 / 勝者S+30 / 敗者硬直40F(勝者追撃確定) / あいこ=仕切直し | ミニゲーム本体の数値はBAL.MINIGAMEに集約(§6.5) |
...
## 6.5. ギュイーン（競り合い）ミニゲーム化（v2.1・v0.7実装対象）

ギュイーン発生ごとに以下3種から1つをランダム選択して遊ばせる。突入演出(`CLASH.inF`=26F)の間は
ルーレットテロップ(「ジャンケン!!」「レンダ!!」「ハヤウチ!!」を高速切替→確定表示)でどれが始まるかを予告する。

### 選択比率

`BAL.MINIGAME.weights`: ジャンケン50% / レンダ25% / ハヤウチ25%。

### ① ジャンケン（既定・§6参照）

ボタンは現行の三角配置のまま。アイコンはマーモットの手: ✊=`drawFist`（既存流用）/ ✋=`drawPalm`（既存流用）/
✌️=新規（爪2本）。解決規則は ✊＞✌️＞✋＞✊（§6の表）。

### ② レンダ（連打の綱引き）

制限時間 `MINIGAME.RENDA.durationF`=120F。プレイヤーはタップ連打、CPUは毎F
`MINIGAME.RENDA.cpuMin`(0.115) + rng()×`MINIGAME.RENDA.cpuRng`(0.02) を自動加算。タップ毎に
`MINIGAME.RENDA.tapGain`分プレイヤー側が加算され、その都度火花+綱引きバー押し込み演出が入る。
120F終了時点で数値が多い方が勝ち、同数は仕切り直し(あいこ)。

### ③ ハヤウチ（的あて連続タップ）

的（半径`MINIGAME.HAYAUCHI.radius`=55px）が1つずつランダム位置に出現し、タップすると即座に
次の的が出る。ミスタップのペナルティ無し。先に`MINIGAME.HAYAUCHI.targets`(5)個を撃ち抜いた方が勝ち。
CPUは仮想的に1個あたり`cpuMinF`〜`cpuMaxF`(14〜24F)で撃破。保険上限`capF`(600F)を超えたら
その時点の撃破数勝負、同数は仕切り直し。進捗はパッド上部に両者分の●で表示。

### 共通リワード

3種とも勝敗が決まった時点の `winner`(0|1|-1)を、既存の `resolveClash` 由来の共通処理
(`applyClashResult`)へ渡す。あいこ/引き分け(-1)は「あいこ! しきりなおし」でノーダメージ。
勝敗時は dmg120+連勝ごと+30 / 勝者ultStock+1・S+30 / 敗者硬直40F(勝者追撃確定)。

### スプライト流用マップ（v0.6のposeIdフォールバックを一部解消）

- レンダ中の両者 → `grab_reach`（押し合いのビジュアルとして流用）
- ジャンケン・ハヤウチ中 → `idle`
- つかみ空振り(`grabrec`) → `hurt`（v0.6では未割当て=図形フォールバックだったが解消）
- ふらつき(`dizzy`) → `hurt`（同上）
- 奥義(`ultAtk`) → 前半(充填中)は`ult_charge`、後半(発動後)は`atk_high`（同上）
