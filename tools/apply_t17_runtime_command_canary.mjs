...
const VALIDATION_OLD=`    const move=moves&&moves[idx];
    if(!move||move.name!==result.decision.name||result.decision.commandId!==f.c.id+':slot-'+(idx+1)){
      throw new Error('Core command canary resolved an unknown current move');
    }`;
const VALIDATION_NEW=`    const move=moves&&moves[idx];
    const compatible=!!move&&(trigger==='grab'
      ?move.type==='grab'
      :(move.type==='atk'||move.type==='stance')&&move.trigger===trigger);
    if(!compatible||move.name!==result.decision.name||result.decision.commandId!==f.c.id+':slot-'+(idx+1)){
...
if(html.includes(VALIDATION_OLD))html=replaceExactlyOnce(html,VALIDATION_OLD,VALIDATION_NEW,'canary compatibility validation');
...
if(count(html,'const compatible=!!move&&(trigger===\'grab\'')!==1)throw new Error('move type/trigger compatibility guard is missing');
